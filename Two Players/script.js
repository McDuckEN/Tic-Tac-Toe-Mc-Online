...
{    
    for(i = 0; i < winRules.length; i++)
    {
        ...
        if(btnA == btnB && btnB == btnC)
        {
            ...
            {              
                PL_Score++;
                lblPLScore.innerText = `امتیاز بازیکن دوم : ${PL_Score}`;
                Gamerunning = false;
            ...
            {
                AI_Score++;
                lblAIScore.innerText = `امتیاز بازیکن اول : ${AI_Score}`;
                Gamerunning = false;
                colored(rule[0],rule[1],rule[2]);
            }
...
function changePlayer()
{
    ...
    {
        currentPlayer = "O";
        lblStatus.textContent = "نوبت : O";
    }
    else
    {
        currentPlayer = "X";
        lblStatus.textContent = "نوبت : X";
    }
}
